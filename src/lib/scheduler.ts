...
class ZurgScheduler {
    ...
    private scheduleInstance(instance: ZurgInstance): void {
        logger.info(`📅 Scheduling instance '${instance.name}' with cron: ${instance.cronSchedule}`);

        if (!cron.validate(instance.cronSchedule)) {
            logger.error(`❌ Invalid cron expression for instance '${instance.name}': ${instance.cronSchedule}`);
            return;
        }
    ...
    private async executeInstance(instance: ZurgInstance): Promise<void> {
        ...
        if (this.runningJobs.has(jobKey)) {
            logger.warn(`⚠️  Instance '${jobKey}' is already running, skipping this execution`);
            return;
        }
        ...
        try {
            ...
            logger.info(`✅ Instance '${instance.name}' completed successfully in ${duration}s`);
        } catch (error) {
            const duration = Math.round((Date.now() - startTime) / 1000);
            logger.error(`💥 Instance '${instance.name}' failed after ${duration}s:`, error);
        } finally {
    ...
    public getStatus(): Array<{
        ...
        return config.instances.map(instance => ({
            ...
            nextRun: this.jobs.get(instance.name)?.nextDate()?.toJSDate(),
        }));
    }

    /**
     * Log next run times for all instances
     */
    private logNextRunTimes(): void {
        ...
        for (const [name, task] of this.jobs) {
            ...
            if (nextRun) {
                logger.info(`  • ${name}: ${nextRun.toISO()}`);
            }
        }
    }
        ...
        const shutdown = (signal: string) => {
            ...
            const waitForJobs = async () => {
                ...
                while (this.runningJobs.size > 0 && waited < maxWait) {
                    logger.info(`⏳ Waiting for ${this.runningJobs.size} running job(s) to complete...`);
                    await new Promise(resolve => setTimeout(resolve, checkInterval));
                    waited += checkInterval;
                }

                if (this.runningJobs.size > 0) {
                    logger.warn(`⚠️  Force shutting down with ${this.runningJobs.size} job(s) still running`);
                }
